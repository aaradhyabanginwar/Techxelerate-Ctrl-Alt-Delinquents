...
def validate_data(data, required_fields):
    ...
    for field, field_type in required_fields.items():
        if field not in data or not isinstance(data[field], field_type):
            return {"message": f"Invalid or missing field: {field}"}, 400
    return None
...
class PatientResource(Resource):
    ...
    def post(self):
        data = request.json
        validation = validate_data(data, {"PatientName": str, "Age": int, "Gender": str, "CG_ID": int})
        if validation:
            return validation
...
class DoctorResource(Resource):
    ...
    def post(self):
        data = request.json
        validation = validate_data(data, {"DoctorNAME": str, "Specialization": str, "StartDate": str})
        if validation:
            return validation
...
class DiagnosesResource(Resource):
    ...
    def post(self):
        data = request.json
        validation = validate_data(data, {"PatientID": int, "Diagnosis": str})
        if validation:
            return validation
...
class DoctorPatientResource(Resource):
    ...
    def post(self):
        data = request.json
        validation = validate_data(data, {"PatientID": int, "Dr_ID": int})
        if validation:
            return validation
...
class ReportResource(Resource):
    ...
    def post(self):
        data = request.json
        validation = validate_data(data, {"Dr_ID": int, "LabID": int, "PatientID": int})
        if validation:
            return validation
